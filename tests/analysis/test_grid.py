...
def test_build_grid_shape_from_max_xy():
    holes = {
        (0, 0): _df([0.0, 1.0, 2.0], 1e-3),
        (0, 1): _df([0.0, 1.0, 2.0], 1e-3),
        (1, 0): _df([0.0, 1.0, 2.0], 1e-3),
    }
...
def test_build_grid_missing_hole_is_nan():
    holes = {(0, 0): _df([0.0, 1.0, 2.0], 1e-3), (1, 1): _df([0.0, 1.0, 2.0], 1e-3)}
    g = build_grid(holes, None, 0.0, 2.0, "X", normalize=False)
...
def test_build_grid_absolute_value():
    holes = {(0, 0): _df([0.0, 1.0, 2.0], 1e-3)}
    g = build_grid(holes, None, 0.0, 2.0, "X", normalize=False)
...
def test_build_grid_normalized_against_reference():
    ...
    holes = {(0, 0): _df([0.0, 1.0, 2.0], 4e-3)}
...
def test_build_grid_origin_zero_is_placed_at_zero():
    """Regression: hole x0-y0 must land at grid[0, 0], not wrap to grid[-1, -1]."""
    holes = {
        (0, 0): _df([0.0, 1.0, 2.0], 1e-3),
        (2, 2): _df([0.0, 1.0, 2.0], 4e-3),
    }
    g = build_grid(holes, None, 0.0, 2.0, "X", normalize=False)
    assert g.shape == (3, 3)
    assert math.isclose(g[0, 0], math.sqrt(2e-3), rel_tol=1e-6)
    assert math.isclose(g[2, 2], math.sqrt(2 * 4e-3), rel_tol=1e-6)
